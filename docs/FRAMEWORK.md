...
| --- | --- | --- |
| Routine implementation | agy `gemini-3.8-flash-medium` | Tier 2: DeepSeek Flash v4.1 under local alias `deepseek-flash` (via `codex exec -p deepseek`) upon Gemini quota exhaustion, with fail-closed monetary-balance preflight; Tier 3 (final): native `implementer`, `gpt-5.6-luna`, medium, on DeepSeek balance depletion/failure or non-quota Gemini failure |
...
Replace `implement` with `review` for Opus. Routine review explicitly passes `--effort medium` to Claude by default. For complex reviews involving coupled contracts, state machines, concurrency, security, or unresolved findings, pass `--review-effort high --review-reason "<reason>"`. A non-empty reason is strictly required for high effort; do not raise review effort solely due to diff size. Review effort and reason flags are rejected for the `implement` role. Selected review effort and reason are recorded in result and dry-run metadata. `--dry-run` inspects planned invocation without contacting a provider. Run via the normal shell tool; if the Codex sandbox cannot access the installed CLI or global quota state, request the normal command escalation. The adapter uses CLI bypass flags for Gemini and Claude as explicitly requested by the user. Nested Codex execution for DeepSeek instead uses unattended `--approve-for-me`, which selects the workspace-write sandbox and must not be combined with an explicit `--sandbox` flag, along with shell secret exclusion (`-c shell_environment_policy.ignore_default_excludes=false`) and an explicit child environment containing only the credential checked by preflight. Gemini, Claude, and Git-evidence subprocess environments omit DeepSeek credentials; Git evidence also disables repository-configured fsmonitor, external diff, and textconv execution. This is defense in depth, not a hard same-user process-containment boundary. These settings do not bypass Codex sandbox or automatic approval review. A CLI or adapter launched through shell is not a native Codex subagent; its logs and structured result are the handoff to the parent.
...
- Exit 20 / `fallback_required`: inspect the result's `fallback` metadata:
  - When Gemini quota is exhausted and DeepSeek Flash is configured (and not opted out via `providers.deepseek.enabled=false`), the adapter routes implementation to DeepSeek Flash (`{"provider": "deepseek", "model": "deepseek-flash"}`).
  - DeepSeek Flash v4.1 (local model alias `deepseek-flash`) performs a live, fail-closed monetary-balance preflight (`GET /user/balance` using `DEEPSEEK_API_KEY`). A preflight HTTP 402, is_available=false, or all-zero balance exits 20 (`fallback_required`) before model launch; unverified preflight (network, invalid API key, 429/5xx, malformed response) exits 1 (`balance_check_failed`). A runtime 402 is only possible after launch and also routes to Luna. To opt out, set `providers.deepseek.enabled = false` in `routing.json` (or remove the entry during v5+ upgrade), which causes Gemini quota exhaustion to fall back directly to Luna medium.
  - When Claude review quota is exhausted, it falls back to native Astra low `reviewer`.
  Carry over the same contract, captured partial changes and prior checks. An active cached result satisfies the provider-first routing requirement; do not launch an exhausted provider again for another operation. For DeepSeek, the adapter starts nested Codex processes (`codex exec -p deepseek`) with unattended `--approve-for-me`; that option selects the workspace-write sandbox, so the adapter does not also pass `--sandbox`.
...
To perform a live check of DeepSeek monetary balance against the official `GET /user/balance` endpoint, pass `--check-live`:

```bash
python3 "$framework_root/scripts/provider_runner.py" status --provider deepseek --check-live --config "$framework_root/routing.json"
```

`status` needs no workspace or task file, creates no run logs, and does not modify state (unless `--check-live` updates the balance snapshot). Gemini and Claude status checks remain strictly local-only and never contact providers. It reports the provider/model, quota/balance file, cached evidence if present, and the appropriate fallback. For a selected provider, `fallback_required` exits 20, `available_to_try` exits 0, and `state_error` or `balance_check_failed` exits 1. Route active blocks and state-read failures to the corresponding native fallback, disclosing the actual reason. Availability means only that the last-known quota record permits an attempt; it does not establish current usage, authentication, model access, or safe file ownership. Normal `implement` and `review` calls enforce the same cache before launching the CLI. Gemini and DeepSeek pending-run checks still apply, and any fallback writer must respect unresolved ownership even when the quota-only status check is blocked.
...
The installer backs up only files it owns/replaces, merges one marked policy block, preserves unrelated configuration and records hashes in `install-manifest.json`. `routing.json` pins absolute CLI paths discovered during setup; rerun the source installer with `--refresh-routing` if the executables move. The DeepSeek route additionally requires a user-level `$CODEX_HOME/deepseek.config.toml` profile whose custom provider uses `env_key = "DEEPSEEK_API_KEY"`; the installer does not create or overwrite personal Codex profiles. Use the Python 3.11+ interpreter available on this machine. See the root README for the profile contents, non-inference validation commands, installation and updates.
...
The adapter normalizes CLI lifecycle telemetry into one provider-neutral contract across Gemini, Claude, and DeepSeek, while preserving each provider's native schema and terminal classification underneath:
...
- DeepSeek requires a validated final status object or a terminal `turn.failed` event.
...
Gemini uses a provider-specific 3600-second soft timeout by default, followed by a 120-second adapter termination grace. Claude and DeepSeek inherit the packaged global 900-second timeout unless an operator sets a provider-specific value; if neither provider nor global timing is present, the code-level fallback is 1800 seconds. During v5-to-v6 upgrade, an untouched legacy global 900-second configuration receives missing Gemini lifecycle defaults field by field; any existing provider-specific timing field is preserved as authoritative. During v6-to-v7 upgrade, the exact packaged 1800-second Gemini default moves to 3600 seconds; any other provider-specific timeout is treated as an operator override and preserved. A custom legacy global timeout is preserved without inferred provider overrides.
