...
Use fresh `refactor_auditor` agents on Luna medium for reuse and efficiency, and `ask-claude` on Claude Opus 5 at medium effort for quality. These three review tasks may run concurrently within host limits. Use the framework's disclosed Astra-low reviewer fallback if Claude is unavailable. Escalate Claude effort only under the framework's stated criteria. Store contracts, saved diffs, and provider output in the active workspace's `.llm-output/`.

Route routine repairs through `ask-gemini` first. Only its confirmed exit-20 quota handoff authorizes the automatic fresh Luna-medium implementer fallback; preserve partial work and ensure the previous writer has stopped. Route complex bounded repairs to Terra low. Assign explicit file ownership, acceptance criteria, and checks; tell writers they share the codebase and must preserve others' edits. Do not authorize recursive delegation.
