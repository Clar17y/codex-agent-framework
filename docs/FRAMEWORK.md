...
## Gemini lifecycle diagnostics

Gemini uses a 1800-second provider soft timeout by default, followed by a 120-second adapter termination grace. It may be legitimately silent while working: wait for the existing adapter command to finish, rather than spawning `Start-Sleep` loops or declaring a provider failure from missing final JSON. The adapter emits low-frequency heartbeat lines and atomically writes each run's `heartbeat.json`. These are diagnostic liveness signals only—`process_alive` means the local CLI remains alive and provider-log activity means its log changed; neither proves semantic progress or backend completion. Native subagents continue to use completion notifications and do not need periodic prose-progress files.

A missing or malformed terminal result is an `uncertain_exit`, not a completed run: its pending record keeps overlapping ownership blocked and sets `fallback_blocked_by_pending` until an operator confirms resolution. Timeout/interruption cleanup records direct-PID observation and termination-attempt outcome only; it cannot verify escaped descendants or provider-backend cessation.

During v5-to-v6 upgrade, an untouched legacy global 900-second configuration receives missing Gemini lifecycle defaults field by field; any existing provider-specific timing field is preserved as authoritative. A custom legacy global timeout is preserved without inferred provider overrides.
