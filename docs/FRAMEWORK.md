...
- Exit 20 / `fallback_required`: inspect the result's `fallback` metadata:
  ...
  Carry over the same contract, captured partial changes and prior checks. An active cached result satisfies the provider-first routing requirement; do not launch an exhausted provider again for another operation. For DeepSeek, the adapter starts nested Codex processes (`codex exec -p deepseek`) with `--sandbox workspace-write` and unattended `--approve-for-me`.
